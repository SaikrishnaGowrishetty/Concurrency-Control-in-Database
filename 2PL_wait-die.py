...
def Read(Tname,Lklist,Txlist):
    ...
    if (Txlist[tid]["Status"] == "Active"):
        ...
        if rsrc in Lklist:
            #if lockmode is write then runs the wait-die by comparing timestamps
            if(Lklist[rsrc]["LockMode"] == "W"):
                ...
                then it blocks the requested transaction, otherwise it aborts it"""
                if(Txlist[tid]["TimeStamp"] < Txlist[tid1]["TimeStamp"]):
                    ...
                    print(tid+" is blocked by wait-die as item "+rsrc+\
                          " is held by "+tid1+" and "+tid+" is older than "+tid1, file=outF)
                else:
                    Txlist[tid]["Status"] = "Aborted"
                    Txlist[tid]["BlockedOperations"] = []
                    Txlist[tid]["BlockedBy"] = "None"
                    print(tid + " tries to get R lock on " + rsrc+" ," + tid + \
                          " gets aborted by wait-die as it is younger than " + tid1, file=outF)
    ...
    if(Txlist[tid]["Status"] == "Active"):
        ...
        if rsrc in Lklist:
            ...
            if(Lklist[rsrc]["LockMode"] == "R" and len(Lklist[rsrc]["TIDList"]) == 1
                ...
                return
            #if rsrc in shared mode then runs the wait-die by comparing timestamps
            else:
                for i in Lklist[rsrc]["TIDList"]:
                    if(i!=tid):
                        ...
                        if(Txlist[tid]["TimeStamp"] < Txlist[i]["TimeStamp"]):
                            print (tid + " is blocked by wait-die as item " + rsrc + \
                                  " is held by " + i + " and " + tid + " is older than " + i,file=outF)
                            ...
                            print (tid + " tries to upgrade R lock on " + rsrc + " to W lock, " + tid + \
                                  " gets aborted by wait-die as it is younger than " + i,file=outF)
                            remove_locks(tid, Lklist, Txlist)
