...
#command for running:- python 2PL_wound_wait.py <inputfilename>
#eg:- python 2PL_wound_wait.py input1.txt
...
def Read(Tname,Lklist,Txlist):
    ...
    if (Txlist[tid]["Status"] == "Active"):
        ...
        if rsrc in Lklist:
            #if lockmode is write then runs the wound-wait by comparing timestamps
            if(Lklist[rsrc]["LockMode"] == "W"):
                tid1 = Lklist[rsrc]["TIDList"][0] #gets transactionid of transaction holding the resource
                """if timestamp of requested transaction is less than the transaction holding the resource
                then it aborts the transaction that is holding the resource. Otherwise, it blocks the requested 
                transaction"""
                if(Txlist[tid]["TimeStamp"] < Txlist[tid1]["TimeStamp"]):
                    #updates the transaction table
                    Txlist[tid1]["Status"] = "Aborted"
                    Lklist[rsrc]["LockMode"] = "R"
                    Lklist[rsrc]["TIDList"] = tid
                    print(tid + " tries to get R lock on " + rsrc+" ," + tid1 + \
                          " gets aborted by wound-wait as it is younger than " + tid, file=outF)
                    remove_locks(tid1,Lklist,Txlist)
                else:
                    # updates the transaction table
                    Txlist[tid]["Status"] = "Blocked"
                    Txlist[tid]["BlockedOperations"].append(Tname)
                    Txlist[tid]["BlockedBy"] = tid1
                    print(tid+" is blocked by wound-wait as item "+rsrc+\
                          " is held by "+tid1+" and "+tid+" is younger than "+tid1, file=outF)
    ...
    if(Txlist[tid]["Status"] == "Active"):
        ...
        if rsrc in Lklist:
            ...
            if(Lklist[rsrc]["LockMode"] == "R" and len(Lklist[rsrc]["TIDList"]) == 1
                ...
                return
            #if rsrc in shared mode then runs the wound-wait by comparing timestamps
            else:
                for i in Lklist[rsrc]["TIDList"]:
                    if(i!=tid):
                        ...
                        then it aborts the transaction that is holding the resource. Otherwise, it blocks the requested 
                        transaction"""
                        if(Txlist[tid]["TimeStamp"] < Txlist[i]["TimeStamp"]):
                            # updates the transaction table
                            Txlist[i]["Status"] = "Aborted"
                            Txlist[i]["BlockedOperations"] = []
                            Txlist[i]["BlockedBy"] = "None"
                            print(tid+" tries to upgrade R lock on "+rsrc+" to W lock, "+i+\
                                  " gets aborted by wound-wait as it is younger than "+tid, file=outF)
                            remove_locks(i,Lklist,Txlist)
                        else:
                            # updates the transaction table
                            print(tid + " is blocked by wound-wait as item " + rsrc + \
                                  " is held by " + i + " and " + tid + " is younger than " + i, file=outF)
                ...
                #upgrades the readlock to write if the requested transaction is older than the shared transactions
                if(Txlist[tid]["Status"] == "Active"):
                    Write_Lock(tid,rsrc,Lklist)
                return
        else:
